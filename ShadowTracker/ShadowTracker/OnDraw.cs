...
namespace ShadowTracker
{
    class OnDraw
    {
        ...
        {            
            if (ShadowTracker.SpellTracker.Spell_Checker.Enemy_Flash_Time > Game.ClockTime) // 점멸
            {
            ...
            if (ShadowTracker.SpellTracker.Spell_Checker.Enemy_Ezreal_Time > Game.ClockTime) // 이즈 비전
            {                
            ...
            if (ShadowTracker.SpellTracker.Spell_Checker.Enemy_Shaco_Time > Game.ClockTime) // 샤코 디시브
            {
            ...
            if (Program.LPet_Time > Game.ClockTime && Program.LPet_Enable == true)  // 르블랑 패시브
            {
                Function.Shadow_Draw(Program.Enemy_Leblanc);
            }

            if (Program.SPet_Time > Game.ClockTime && Program.SPet_Enable == true)  // 샤코 궁
            {
                Function.Shadow_Draw(Program.Enemy_Shaco);
            }

            if (ShadowTracker.SpellTracker.Spell_Checker.Enemy_Monkey_Enable == true)   // 오공 디코이
            {
